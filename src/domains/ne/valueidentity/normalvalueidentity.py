...
class Net:
    ...
    def put(self,sample):
        ...
        # 如果网络能耗高于上限，执行网络重构操作
            ...
            for leaf in self.leafs:
                if len(leaf.samples) <= 1:continue
                den = leaf.width/len(leaf.samples)
                if den < Net.node_sample_density:
                    self.__spilt(leaf)
                    self.spiltcount += 1
                    action = '分裂(样本密度过大)'
    ...
    def draw(self):
        ...
        tree_graph.write('nettree.dot')
        tree_graph.layout('dot')  # layout with dot
        tree_graph.draw('nettree.png')  # write to file
