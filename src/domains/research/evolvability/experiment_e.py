...
datapath = files.get_data_path() + os.sep + 'evolvability' + os.sep + 'experimentE' + os.sep
...
def callback(event,monitor):
    ...
    global  mode
    global  maxfitness
    global  maxfitness_ind
    if event == 'epoch.end':
        gc.collect()
        fitness = monitor.evoTask.curSession.pop.inds[0]['fitness']
        fitness_ind = monitor.evoTask.curSession.pop.inds[0]
        maxfitness_records.append(fitness)
        if fitness > maxfitness:
            maxfitness_ind = fitness_ind
            maxfitness = fitness

        # 如果最大适应度达到了env.max_notdone_count（说明对当前环境已经产生适应），或者进化迭代数超过10次（当前环境下适应达到最大）
        # 则提升复杂度
        epochcount += 1
        if fitness >= env.max_notdone_count or epochcount >= maxepochcount:
            # 保存复杂度和对应最大适应度记录
            ...
            # 计算最优网络模块度
            modularity,parts = maxfitness_ind.genome.compute_modular()
            modularities.append((complex_records[-1],modularity))
            print(modularities)
            print(parts)
            filename = datapath + 'modularity.csv'
            out = open(filename, 'a', newline='')
            csv_write = csv.writer(out, dialect='excel')
...
def execute():
    ...
    runParam = {
        ...
        'log':{
            ...
            'file': datapath+'neat_cartpole.log'                            # 日志文件名
        },
        'evalate':{
...
if __name__ == '__main__':
    ...
    force.init()
    run(mode='noreset',maxepochcount=30,complexunit=5.)
