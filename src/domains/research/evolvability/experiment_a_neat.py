...
neatdatapath = files.get_data_path()+os.sep + 'evolvability' + os.sep + 'experimentA' + os.sep + 'neat' + os.sep
...
def callback(event,monitor):
    ...
    if event == 'epoch.end':
        ...
        if maxfitness >= env.max_notdone_count or epochcount >= maxepochcount:
            ...
            # 保存过程中每一步的最大适应度记录
            filename = neatdatapath + 'neat_' + mode + '_' + str(exec_xh) + '.csv'
            ...
            netviewer.drawNet(epoch_maxfitness_ind.genome, filename=filename, view=False)
            ...
            print(modularities)
            filename = neatdatapath + 'neat_' + mode + '_' + str(exec_xh) + '_' + 'modularity.csv'
            out = open(filename, 'a', newline='')
            csv_write = csv.writer(out, dialect='excel')
...
def run(**kwargs):
    ...
    maxepochcount = 30 if 'maxepochcount' not in kwargs else int(kwargs['maxepochcount'])
    complexunit = 20.0 if 'complexunit' not in kwargs else float(kwargs['complexunit'])
    exec_xh = 0 if 'xh' not in kwargs else int(kwargs['xh'])
...
def execute():
    ...
    runParam = {
        ...
        'log':{
            ...
            'file': neatdatapath+'neat_cartpole.log'              # 日志文件名
        },
        'evalate':{
