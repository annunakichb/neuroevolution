...
from evolution.operation import SelectionBase
import utils.strs as strs

######################################
#  (NSGA-II)实现，修改自https://github.com/annunakichb/deap/blob/master/deap/tools/emo.py  #
class NSGA2(SelectionBase):
    name = 'NSGA2'

    def __init__(self):
        self.name = NSGA2.name
    ...
    .. [Deb2002] Deb, Pratab, Agarwal, and Meyarivan, "A fast elitist
       ...
       optimization: NSGA-II", 2002.
        ...
        :return:
        '''
        individuals = session.pop.inds  #获得所有个体
        k = int(len(session.pop.inds)*0.8)
        nd = 'standard'
...
class selNSGA3WithMemory(NSGA2):
    '''
    ...
    def __call__(self, individuals, k):
        chosen, memory = self.selNSGA3(individuals, k, self.ref_points, self.nd,
    ...
    def selNSGA3(self, individuals, k, ref_points, nd="log", best_point=None,
        ...
        extreme_points = self.find_extreme_points(fitnesses, best_point, extreme_points)
        front_worst = np.max(fitnesses[:sum(len(f) for f in pareto_fronts), :], axis=0)
        intercepts = self.find_intercepts(extreme_points, best_point, worst_point, front_worst)
        niches, dist = self.associate_to_niche(fitnesses, ref_points, best_point, intercepts)
        ...
        n = k - sel_count
        selected = self.niching(pareto_fronts[-1], n, niches[sel_count:], dist[sel_count:], niche_counts)
    ...
    def find_extreme_points(self, fitnesses, best_point, extreme_points=None):
    ...
    def find_intercepts(self, extreme_points, best_point, current_worst, front_worst):
    ...
    def associate_to_niche(self, fitnesses, reference_points, best_point, intercepts):
        ...
        distances = distances[:, :, np.newaxis] * reference_points[np.newaxis, :, :] / norm[np.newaxis, :, numpy.newaxis]
    ...
    def niching(self, individuals, k, niches, distances, niche_counts):
        selected = []
